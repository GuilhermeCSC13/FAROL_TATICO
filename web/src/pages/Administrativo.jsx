...
export default function Administrativo() {
  ...
  const subsetor = useMemo(() => AREAS[area] || AREAS["7"], [area]);
  const contentKey = area + "-" + aba;

  const renderContent = () => {
    if (aba === "metas") return <AdministrativoMetas key={contentKey} />;
    if (aba === "rotinas") return <AdministrativoRotinas key={contentKey} />;
    return <AdministrativoResumo key={contentKey} />;
  };
      ...
      <div className="h-full overflow-hidden bg-slate-100 p-6 flex flex-col gap-5">
        <section className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.28em] text-blue-600">Administrativo</p>
